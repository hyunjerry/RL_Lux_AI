...
# 11, 13, 11, 14
model0 = torch.jit.load(f'{path}/model_0_11_epochs_accu_0.84_loss_0.42.pth')
model1 = torch.jit.load(f'{path}/model_1_15_epochs_accu_0.84_loss_0.43.pth')
model2 = torch.jit.load(f'{path}/model_2_11_epochs_accu_0.82_loss_0.47.pth')
model3 = torch.jit.load(f'{path}/model_3_14_epochs_accu_0.82_loss_0.47.pth')
...
def agent(observation, configuration):
    ...
    with torch.no_grad():
        for model in models:
            ...
            temp_policy = p.squeeze(0).numpy()
            if final_policy == None:
                final_policy = np.zeros_like(temp_policy)
            final_policy += temp_policy
        # p0 = model0(torch.from_numpy(state_1).unsqueeze(0), torch.from_numpy(state_2).unsqueeze(0))
        # policy0 = p0.squeeze(0).numpy()
        # p1 = model1(torch.from_numpy(state_1).unsqueeze(0), torch.from_numpy(state_2).unsqueeze(0))
        # policy1 = p1.squeeze(0).numpy()
        # p2 = model2(torch.from_numpy(state_1).unsqueeze(0), torch.from_numpy(state_2).unsqueeze(0))
        # policy2 = p2.squeeze(0).numpy()
        # p3 = model3(torch.from_numpy(state_1).unsqueeze(0), torch.from_numpy(state_2).unsqueeze(0))
        # policy3 = p3.squeeze(0).numpy()
        
        #final_policy = policy0 + policy1 + policy2 + policy3

    for unit in player.units:
        if unit.can_act() and (game_state.turn % 40 < 30 or not in_city(unit.pos)):
            action, pos = get_action(final_policy, unit, dest, shift)
